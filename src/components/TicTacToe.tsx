...
function checkWinner(board: BoardArray): string | null {
  const lines = [
    // Rows
...
export default function TicTacToe() {
  ...
  function handleOnClick(row: number, col: number) {
    ...
    // Computer's move
    if (!newWinner) {
      const [computerRow, computerColumn] =
        makeComputerMove(updatedPlayerBoard);
      const updatedComputerBoard = updatedPlayerBoard.map((newRow, rowIndex) =>
        newRow.map((cell, cellIndex) =>
          rowIndex === computerRow && cellIndex === computerColumn ? "O" : cell
        )
      );
